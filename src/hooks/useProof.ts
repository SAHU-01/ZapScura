...
export function useProof() {
  ...
  const prove = useCallback(async (input: { type: CircuitType; data: unknown }): Promise<{ proof: Uint8Array; publicInputs: string[] }> => {
    setIsProving(true);
    setError(null);
    setResult(null);
    ...
    try {
      ...
      return { proof: proofResult.proof, publicInputs: proofResult.publicInputs };
    } catch (err) {
